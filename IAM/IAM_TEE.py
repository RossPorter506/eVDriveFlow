...
# Get hash of files in current working directory. This will either be ../evcc or ../secc when
# run by the EVCC or SECC respectively.
def hash_sign_software(nonce: int) -> (bytes, bytes):
...
if __name__ == "__main__":
    ...
    (secc_hash, _) = hash_sign_secc_software(0)
    
    # Measure ../evcc folder
    os.chdir(os.path.join(os.path.pardir, "evcc"))
    (evcc_hash, _) = hash_sign_secc_software(0)
    
    os.chdir(os.path.join(os.path.pardir, "IAM"))
    with open("secc.sha256", "wb") as f:
        f.write(secc_hash)
    with open("evcc.sha256", "wb") as f:
        f.write(evcc_hash)
